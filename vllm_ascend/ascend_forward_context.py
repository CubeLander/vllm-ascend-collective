...
def _moe_phase_hybrid_non_decode_fused_available(vllm_config: VllmConfig, num_tokens: int) -> bool:
    ...
    FUSED_MC2 / dispatch_ffn_combine BF16 path, and additionally enforces
    the PR #203 token boundary ``num_tokens <= mc2_tokens_capacity``: a
    fused wave must fit the MC2 token capacity exactly like every other
    MC2-family route. A requested fused phase that exceeds capacity is not
    silently rerouted; :func:`select_moe_comm_method` raises fail-fast
    instead.
    """
...
def _moe_phase_hybrid_non_decode_fused_error(
    vllm_config: VllmConfig, phase: MoEForwardPhase, num_tokens: int
) -> ValueError:
    ...
    message names the gate requirement (including the MC2 token capacity,
    which the policy enforces fail-fast exactly like every other MC2-family
    route) so the operator can fix the configuration (or turn the policy
    off) instead of discovering a nondeterministic runtime mismatch.
    """
    ...
    return ValueError(
        ...
        f"num_tokens={num_tokens}, mc2_tokens_capacity={get_mc2_tokens_capacity()}). "
        ...
        ">=3 local experts; every phase: num_tokens within "
        "mc2_tokens_capacity) or set moe_phase_hybrid_policy=off."
    )
...
def select_moe_comm_method(
    ...
    *,
    ...
    - ``PURE_PREFILL`` and ``MIXED`` select ``MoECommType.FUSED_MC2``; when
      the static SoC capability gate does not hold -- or the wave exceeds
      ``mc2_tokens_capacity`` -- selection **raises a deterministic
      ``ValueError``** (fail fast) instead of falling back to the
      token-dependent baseline family -- a baseline non-decode wave inside
      the single fused torch.compile lane is exactly the stale-template
    ...
    With ``MoEPhaseHybridPolicy.OPAQUE_FUSED_CONTROL`` (measurement-only)
    the same matrix applies but ``PURE_DECODE`` also routes through
    ``FUSED_MC2``, so every phase shares the same fused capability gate
    (including the token capacity) and raises fail-fast when it does not
    hold.
    """
    policy = get_moe_phase_hybrid_policy(vllm_config)
    if policy == MoEPhaseHybridPolicy.OFF or phase is None or is_draft_model or not is_moe_model(vllm_config):
    ...
    if wants_fused:
        if _moe_phase_hybrid_non_decode_fused_available(vllm_config, num_tokens):
            logger.debug(
        ...
        raise _moe_phase_hybrid_non_decode_fused_error(vllm_config, phase, num_tokens)
...
def _select_moe_comm_method_stock(num_tokens: int, vllm_config: VllmConfig, is_draft_model=False) -> MoECommType | None:
    ...
    3. On A2 with expert parallel, use the BF16 or W8A8 dynamic fused MC2 path
       ...
       for the kernel pipeline and tokens fit the MC2 capacity. Otherwise,
       pick MC2 when tokens fit the MC2 capacity and the DP size is large
       enough, or fall back to all-gather.
